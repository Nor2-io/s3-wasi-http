...
use super::{
    checksum_algorithm_from_str, parse_xml_bool, parse_xml_string, parse_xml_value, x_amz_headers::{
        storage_class_from_str, 
        XAmzStorageClass
    }, ApiChecksumType, ApiObject, ApiOwner, ApiRestoreStatus, S3RequestBuilder, S3RequestData, S3ResponseData
};
        ...
        loop {
            ...
            match element {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "IsTruncated" => {
                    list_objects_response.is_truncated = parse_xml_bool(&mut parser, "IsTruncated")?;
                },
                ...
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "Delimiter" => {
                    list_objects_response.delimiter = Some(parse_xml_string(&mut parser, "Delimiter")?);
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "MaxKeys" => {
                    list_objects_response.max_keys = parse_xml_value::<i32>(&mut parser, "MaxKeys")?;
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "EncodingType" => {
                    list_objects_response.encoding_type = Some(parse_xml_string(&mut parser, "EncodingType")?);
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "KeyCount" => {
                    list_objects_response.key_count = parse_xml_value::<i32>(&mut parser, "KeyCount")?;
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "ContinuationToken" => {
                    list_objects_response.continuation_token = Some(parse_xml_string(&mut parser, "ContinuationToken")?);
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "NextContinuationToken" => {
                    list_objects_response.next_continuation_token = Some(parse_xml_string(&mut parser, "NextContinuationToken")?);
                },
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "StartAfter" => {
                    list_objects_response.next_continuation_token = Some(parse_xml_string(&mut parser, "StartAfter")?);
                },
                ...
                xml::reader::XmlEvent::StartElement { name, .. } if name.local_name == "Contents" => {
                    list_objects_response.contents.push(ApiObject::parse(&mut parser)?);
                },
